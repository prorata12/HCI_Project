...
import copy
def file_path_generator(path, _filelist):
    filelist = copy.deepcopy(_filelist)
    for idx,filename in enumerate(filelist):
        filelist[idx] = path + '/' + filename
    return filelist

# These contain File Path
f1_train_list = file_path_generator(f1_train_path, os.listdir(f1_train_path))
f1_test_list = file_path_generator(f1_test_path, os.listdir(f1_test_path))
f2_train_list = file_path_generator(f2_train_path, os.listdir(f2_train_path))
f2_test_list = file_path_generator(f2_test_path, os.listdir(f2_test_path))
